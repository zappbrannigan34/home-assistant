...
Hybrid long-horizon CO2 ventilation control. НЕ PID — используется 30-минутное сглаживание CO2, 30-минутное изменение как длинный тренд, переменный шаг по величине ошибки и явный 30-минутный cooldown между реальными движениями привода.
...
| `input_datetime.ventilation_last_adjustment` | Input | Последняя реальная регулировка |

### Automations

| Automation | Purpose |
|------------|---------|
| `ventilation_control_main` | Применяет рекомендуемую позицию к приводу (rate limit 30 минут, блокировка по холоду) |
