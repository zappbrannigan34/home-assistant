...
## Existing Package: `ventilation` (v1.1.0)

**Device:** Drivent V2 (WindowMaster chain actuator) — `cover.drivent_zap_left_driventokonnyi_privod`

Hybrid long-horizon CO2 ventilation control. НЕ PID — используется 30-минутное сглаживание CO2, 30-минутное изменение как длинный тренд, подтверждение умеренных зон и явный 30-минутный cooldown между реальными движениями привода.
...
- Подтверждение умеренных зон на 2 последовательных 30-минутных окна
...
| Moderate confirmation | 2 windows | Умеренные зоны требуют 2 подряд подтверждённых 30-минутных окна |
