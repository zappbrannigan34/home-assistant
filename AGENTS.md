...
## Existing Package: `ventilation` (v1.3.0)

**Devices:** Drivent V2 (WindowMaster chain actuators) — grouped `zap` + single `eva`

Hybrid long-horizon CO2 ventilation control for two rooms (`zap`, `eva`). Recommendation is now computed continuously from filtered CO2, slope, and 30-minute forecast error, while actuator movement remains separately gated every 30 minutes.
...
- dead zone ±50 ppm is applied on forecast error with hysteresis
