...
## Existing Package: `ventilation` (v1.2.0)

**Devices:** Drivent V2 (WindowMaster chain actuators) — grouped `zap` + single `eva`

Hybrid long-horizon CO2 ventilation control for two rooms (`zap`, `eva`). НЕ PID — используется 30-минутное сглаживание CO2, 30-минутное изменение как длинный тренд, переменный шаг по величине ошибки и явный 30-минутный cooldown между реальными движениями привода.
...
- Длинный тренд `sensor.co2_trend` с порогом ±30 ppm за 30 минут
- Гибридное управление с переменным шагом 5/10/20/30% по модулю ошибки
- Подтверждение только в малой зоне около deadband
...
| `sensor.ventilation_co2_error` | Template | Ошибка: сглаженный CO2 − target (ppm) |
...
| `sensor.eva_co2_trend` | Template | Тренд CO2 eva |
| `sensor.ventilation_eva_recommended_position` | Template | Рекомендуемая позиция окна eva |
| `sensor.ventilation_eva_co2_error` | Template | Ошибка: сглаженный CO2 eva − target |
