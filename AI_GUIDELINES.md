...
Каждый package в `packages/<name>/` имеет свой локальный `README.md` с описанием алгоритма, entity, ограничений и порядка установки.

**Обязательное правило:** перед любыми изменениями внутри конкретного package сначала прочитать его локальный `README.md`, а уже потом править YAML, дашборды или деплой-путь для этого package.
...
└── packages/
    ...
    └── ventilation/
        ├── ventilation_control.yaml # ZAP generation 5 setpoint tracking (v2.4.0)
        └── README.md               # Документация пакета
```
...
## Existing Package: `ventilation` (v2.4.0)

**Devices:** Drivent V2 (WindowMaster chain actuators) — grouped `zap` + single `eva`

ZAP использует room-local cycle-identified PI controller generation 5 с минутным расчётом recommendation, 10-минутным forecast assist, smooth low-CO₂ ceiling, setpoint-headroom temperature regulation и отдельным 30-минутным actuator gate. EVA сохраняет прежний controller.
...
- `sensor.ventilation_zap_thermal_cap`: predicted room temperature, heating setpoint, 2°C setpoint headroom band, outdoor heat-loss risk и last-known-good setpoint fallback; cap=`min_position` при прогнозе на setpoint или ниже
...
- отдельные включительные hard thresholds: `ventilation_min_indoor_temp` и `ventilation_min_outdoor_temp`; они не участвуют в soft setpoint regulation
...
| `cover.ventilation_zap_group` | Device | Логическая группа окон zap |
...
| `cover.drivent_eva_driventokonnyi_privod` | Device | Физический привод eva |
| `binary_sensor.drivent_zap_left_datchik_peregruzki` | Device | Датчик перегрузки привода |
| `binary_sensor.drivent_zap_back_datchik_peregruzki` | Device | Датчик перегрузки zap back |
| `binary_sensor.drivent_eva_datchik_peregruzki` | Device | Датчик перегрузки eva |
| `sensor.drivent_zap_left_datchik_dozhdia` | Device | Датчик дождя (%) |
| `weather.forecast_home_assistant` | Source | Погода (attr: temperature) |
| `sensor.ventilation_co2_mean_30m` | Statistics | Сглаженный CO2 за 30 минут |
| `sensor.ventilation_co2_change_30m` | Statistics | Изменение CO2 за 30 минут |
...
| `sensor.ventilation_eva_co2_filtered` | Filter | Continuous filtered CO2 eva |
| `sensor.ventilation_eva_co2_slope` | Derivative | Сlope по filtered CO2 eva |
| `sensor.ventilation_eva_forecast_co2_30m` | Template | Прогноз CO2 eva на 30 минут |
| `sensor.ventilation_eva_forecast_error_30m` | Template | Forecast error eva на 30 минут |
| `sensor.ventilation_eva_recommended_position` | Template | Continuous recommended opening eva |
| `sensor.ventilation_eva_co2_error` | Template | Ошибка контроллера eva = forecast error |
...
| Automation | Purpose |
|------------|---------|
| `ventilation_control_main` | Применяет рекомендуемую позицию к grouped zap cover |
| `ventilation_safety_close` | Аварийное закрытие zap: холод, перегрузка, потеря CO2 |
| `ventilation_device_error` | Уведомление о перегрузке приводов zap |
| `ventilation_eva_control_main` | Применяет рекомендуемую позицию к приводу eva |
| `ventilation_eva_safety_close` | Аварийное закрытие eva: холод, перегрузка, потеря CO2 |
| `ventilation_eva_device_error` | Уведомление о перегрузке привода eva |
...
Эти правила обязательны и не являются optional hints.
