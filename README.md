...
|-------|----------|--------|
| [humidity](packages/humidity/) | Адаптивное управление увлажнителем с учётом разницы температур улица/комната. Целевая влажность автоматически подстраивается: зимой (-30°C) → 38%, летом (+20°C) → 50%. Решает проблему падения влажности при проветривании (холодный воздух содержит мало воды). Подробнее: [README](packages/humidity/README.md) | ✅ Работает |
...
- **Auto-recovery после reload/restart**: Автоматическая инициализация устройства при перезагрузке YAML или Home Assistant
